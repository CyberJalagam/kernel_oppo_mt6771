		      ...
		      uintptr_t *fun_addr, uintptr_t *end_addr)
{
	...
	elf_len = len;
	/*check signal*/
	ret = qh_verify_sig((void *)buf, len, &elf, (unsigned long *)&elf_len);
	if (sign_flag && (ret != 0)) {
...
/*  end */
	...
	if (IS_ERR(tfm)) {
		pr_err("patch Unable to allocate struct crypto_hash\n");
		goto free_class;
	}
	desc.tfm = tfm;
	desc.flags = 0;
	...
	if (ret < 0) {
		pr_err("patch crypto_hash_init() failed\n");
		crypto_free_hash(tfm);
		goto free_class;
	}
	sg_init_one(&sg, version, strlen(version));
	ret = crypto_hash_update(&desc, &sg, strlen(version));
	if (ret < 0) {
		pr_err("patch crypto_hash_update() failed for id\n");
		crypto_free_hash(tfm);
		goto free_class;
	}
	...
	pr_debug("patch init uts info =%s,digest =%s,len=%d\n",version, version_digest, (int)strlen(version));
	crypto_free_hash(tfm);

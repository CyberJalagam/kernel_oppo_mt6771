...
void mem_block_writeable(unsigned long addr)
{
	...
	if ((mem_block.saved_pmd & PMD_TYPE_MASK) != PMD_TYPE_SECT)
		return;
	*mem_block.pmd &= ~(1 << 15);
...
static inline void _qihoo_patch_register(struct qihoo_patch_object *_qpobj)
{
	...
	case e_module_patch_sync_break: {
		...
		if (mod) {
			__module_get(mod);
			set_page_attributes(mod->module_core, mod->module_core + mod->core_text_size, set_memory_rw);
			__register_break_handler(_qpobj, 0);
			set_page_attributes(mod->module_core, mod->module_core + mod->core_text_size, set_memory_ro);
		}
		break;
	}
#endif
